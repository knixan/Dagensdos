...
export default function SettingsForm({ user }: SettingsFormProps) {
  const emailForm = useForm<EmailValues>({
    defaultValues: { email: user?.email ?? '', confirmEmail: user?.email ?? '' },
    mode: 'onTouched',
  })
    ...
    <div className="space-y-8">
      <section className="space-y-4">
        <h3 className="text-lg font-semibold flex items-center gap-2">
          ...
          {user?.role === 'admin' && (
            <span className="ml-2 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">Admin</span>
          )}
        </h3>
        ...
        <form onSubmit={handleEmail(onSubmitEmail)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <label className="flex flex-col">
            ...
            <Input {...regEmail('email', { required: 'E-post krävs', pattern: { value: /^\S+@\S+$/i, message: 'Ogiltig e-post' } })} type="email" />
          ...
          <label className="flex flex-col">
            ...
            <Input {...regEmail('confirmEmail', { required: 'Bekräfta e-post', validate: (v) => v === getEmailValues('email') || 'E-post adresserna matchar inte' })} type="email" />
      ...
      <section className="space-y-4">
        ...
        <form onSubmit={handlePassword(onSubmitPassword)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <label className="flex flex-col">
            ...
            <Input {...regPassword('password', { required: 'Lösenord krävs', minLength: { value: 8, message: 'Minst 8 tecken' } })} type="password" placeholder="Minst 8 tecken" />
          ...
          <label className="flex flex-col">
            ...
            <Input {...regPassword('confirmPassword', { required: 'Bekräfta lösenord', validate: (v) => v === getPasswordValues('password') || 'Lösenorden matchar inte' })} type="password" />
